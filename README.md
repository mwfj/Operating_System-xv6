...
+ **Project2**: Null pointer and share memory
	- **Part A**: Null-pointer Dereference: In xv6, the VM system uses a simple two-level page table as discussed in class. As it currently is structured, user code is loaded into the very ﬁrst part of the address space. Thus, if you dereference a null pointer, you will not see an exception (might not as you might expect as in Linux); rather, you will see whatever the the pointer in the your program points to in xv6, it will not trigger the nullpointer exception. Thus, the thing you might do is create a program that dereferences a null pointer.
